"""
OFI_daily_slope_analyzer.py  — 日足版
ローリング傾き計算 + ルールベース異常検出 + K-meansクラスタリング
ウィンドウ: 5日（1週間）・20日（1ヶ月）
"""
...
if __name__ == "__main__":
    ...
    df_out.to_csv("ofi_daily_slope_result.csv", index=False)

    print("[OK] 日足傾き分析完了: ofi_daily_slope_result.csv")
    for w in WINDOWS:
        n = int(df_out[f"stealth_{w}d"].sum())
        print(f"  ステルス買い検出 ({w}日窓): {n} / {len(df_out)} 日")
    print("\n  K-meansクラスタ:")
    for k, name in cluster_map.items():
        n = (df_out["cluster"] == k).sum()
        print(f"    Cluster {k}: {name} ({n}日)")
